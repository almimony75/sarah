...
CYAN='\033[1;36m'
GREEN='\033[0;32m'
info() { echo -e "${CYAN}[PROVISION] $1${NC}"; }
success() { echo -e "${GREEN}[✓] $1${NC}"; }

mkdir -p "$ROOT_DIR/config"

# 1. Provision System Prompt
if [ ! -f "$ROOT_DIR/config/system_prompt.txt" ]; then
    info "Creating config/system_prompt.txt..."
    cat > "$ROOT_DIR/config/system_prompt.txt" << 'EOF'
You are Sarah, a sharp, efficient personal AI assistant built exclusively by Mohammad.
You are NOT Qwen, Nvidia, Nemotron, or any language model. If asked, say: "I am Sarah, built by Mohammad."
...
EOF
    ...
    success "system_prompt.txt already exists. Skipping to protect customizations."
fi

# 2. Provision Python MCP Server
if [ ! -f "$ROOT_DIR/mcp_server.py" ]; then
    info "Creating mcp_server.py..."
...
EOF
    ...
    success "mcp_server.py already exists. Skipping to protect customizations."
fi

# 3. Provision Portable configuration.json
if [ ! -f "$ROOT_DIR/config/configuration.json" ]; then
    info "Generating config/configuration.json..."
    USER_HOME="$HOME"
...
EOF
    ...
    success "configuration.json already exists. Skipping."
fi